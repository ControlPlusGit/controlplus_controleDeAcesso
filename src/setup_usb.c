...
void commandHandlerPortaUSB(void){
    ...
    // VERIFICA SE Ã‰ UM COMANDO VALIDO
    if(strncmp(bufferRxUSB, "#CMD", 4) == 0){
        memset(strCMD, NULL, 10);
        strCMD[0] = bufferRxUSB[4];
        strCMD[1] = bufferRxUSB[5];
        //strCMD[2] = '\0'; // fim da string
        comando = atoi(strCMD); // Converte String em NÃºmero int
                
        // VERIFICA SE Ã‰ COMANDO PARA TERAR PARAMETROS
        if(bufferRxUSB[6] == ';'){
            switch(comando){
        ...
        // VERIFICA SE Ã‰ COMANDO PARA OBTER PARAMETROS SALVOS (QUERY)
        if(bufferRxUSB[6] == '?'){
            switch(comando){
                case CMD_MODO_DE_OPERACAO:
                    obtemModoDeOperacao();
